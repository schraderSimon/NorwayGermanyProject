...
def plotwind():
    ...
    ax.set_title("Wind, n=%d, year=%d"%(num_simulations,start_year))
    ...
    sns.kdeplot(german_wind_surplus,color="red",bw_adjust=0.4,alpha=0.5,ax=ax)

    ax.hist(german_wind_surplus,bins=200,density=True,range=(0,cutoff*2),label="Total wind overproduction",color="red",alpha=0.5)
    bins=int(200/cutoff*np.max(german_wind_toNorway))
    ax.hist(german_wind_toNorway,bins=bins,density=True,label="Exportable wind overproduction",color="blue",alpha=0.5)
    if start_year==2020:
        sns.kdeplot(german_wind_toNorway,color="blue",bw_adjust=0.2,alpha=0.5,ax=ax)
    else:
        sns.kdeplot(german_wind_toNorway,color="blue",bw_adjust=0.4,alpha=0.5,ax=ax)
    ax.set_xlabel(r"TWh")
...
def plotProductionDistr():
    ...
    print(data_pandas)
    for i in range(6):
        sns.kdeplot(data[:,i],label=labels[i],color=colors[i],ax=ax2)
    ax2.set_title("2020")
    #ax2.legend()
    ax2.set_xlabel("TWh")
    data=[]
    ...
    for i in range(6):
        sns.kdeplot(data[:,i],label=labels[i],color=colors[i],ax=ax1)
    ax1.set_title("2022")
    ax1.set_xlabel("TWh")
    handles, labelss = ax2.get_legend_handles_labels()
    fig.legend(handles, labelss, bbox_to_anchor=(1.0,0.7))
    fig.tight_layout()
...
def plotMAIN():
    ...
    sns.kdeplot(exp_balance_case1,label="NO net el. export, main",color="blue",ax=ax1)
...
def plot2():
    ...
    ax1.set_title("Electricity, n=%d, year=%d"%(num_simulations,start_year))
    sns.kdeplot(nor_balance_case0,label="NO el. surplus, baseline",color="red",ax=ax1)
    sns.kdeplot(exp_balance_case2,label="NO net el. Export, case 2",color="blue",ax=ax1)
...
def plotPLATFORM():
    ...
    #The error is here. I call it "reduction, but in actuality, it is the "difference".
...
def plotDEPLATFORM():
    ...
    sns.kdeplot(toGermany_case3_3,x=r"Million tons CO$_2$",label="Sent to DE",color="red",ax=ax2)
    sns.kdeplot(toPlatforms_case3_3,x=r"Million tons CO$_2$",label="Sent to Pltf.",color="orange",ax=ax2)
    ax2.set_xlabel("TWh")
    ax2.legend()
    ax1.hist2d(toGermany_case3_3,toPlatforms_case3_3,bins=50,cmap="PuRd")
    ax1.set_facecolor(background)

    ax1.set_xlabel("Sent to DE")
    ax1.set_ylabel("Sent to Pltf.")
